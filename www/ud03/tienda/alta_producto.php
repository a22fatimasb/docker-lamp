...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
    if (empty($_POST["name"]) || empty($_POST["descripcion"]) || empty($_POST["precio"]) || empty($_POST['unidades']) || empty($_FILES['foto']['name'])) {
        ...
        if (empty($mensajes)) {
            // Extraer el nombre de archivo de la ruta proporcionada en $foto["name"]
            ...
            if(file_exists(($directorio))){
                ...
                if(move_uploaded_file($foto["tmp_name"], $directorio)){
                    ...
                    $conexion = establecerConexion();
                    seleccionarBaseDeDatos($conexion);
                    altaProducto($conexion, $nombre, $descripcion, $precio, $unidades, $directorio);
